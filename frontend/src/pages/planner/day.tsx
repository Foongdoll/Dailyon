      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[240px_1fr]">
          <aside className="rounded-3xl border border-slate-800 bg-slate-900/60 p-4">
            <h2 className="mb-3 text-sm font-semibold text-white">타임 테이블</h2>
            <div className="space-y-3">
              {hours.map((hour) => (
                <div key={hour} className="flex items-center gap-2 text-xs text-slate-400">
                  <Clock className="h-3.5 w-3.5" /> {String(hour).padStart(2, "0")}:00
                </div>
              ))}
            </div>
          </aside>
