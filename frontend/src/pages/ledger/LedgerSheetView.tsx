...
import type { SheetMeta } from "./LedgerSheetList.tsx";
...
export default function LedgerSheetView({
  ...
  return (
    <section className="flex max-h-[50%] min-h-[calc(40vh)] items-center justify-center bg-slate-950 px-6 py-8 text-slate-100 md:px-8">
      <div className="w-full max-w-[90rem] overflow-hidden rounded-3xl border border-slate-800 bg-slate-900/60 shadow-2xl shadow-black/40">
        ...
        <div className="flex items-center justify-between border-b border-slate-800/60 bg-slate-900/80 px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-slate-300/90">{sheet.name}</span>
            <span className="text-xs text-slate-500">({sheet.rows}×{sheet.cols})</span>
          </div>
          <div className="flex items-center gap-2">
            <button className="rounded-lg border border-white/10 bg-white/5 p-2 hover:bg-white/10" title="설정">
        ...
        <div className="flex items-stretch gap-2 border-b border-slate-800/60 bg-slate-900/50 px-3 py-2">
          ...
          <input
            placeholder="선택된 셀의 수식 또는 값이 여기에 표시됩니다"
        ...
        <div className="relative max-h-[60vh] overflow-auto custom-scrollbar">
          <div className="min-w-max">
            {/* Column header row */}
            <div className="sticky top-0 z-20 grid grid-cols-[56px_repeat(26,140px)] border-b border-slate-800/60 bg-slate-900/95">
            ...
            {/* Scrollable cells */}
            <div className="grid grid-cols-[56px_repeat(26,140px)]">
              {rows.map((r) => (
